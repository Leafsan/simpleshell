...
4. 명령문 구분
  파싱된 명령문을 받은 실행자는 해당 명령문이 built-in 명령문인지 확인하고 built-in일 경우에는 실행한다.
...
## 결론
이전에 한번 만든 셸 이상으로 충분히 만족스러운 결과가 나왔다고 생각한다.

## 교수님 피드백
우선 exit과 같은 내부 명령어는 바이너리 형태로 파일 시스템에 존재하지 않기 때문에 fork()와 exec()를 사용해서 실행할 수 없다. 둘째, 백그라운드로 실행했을 때 부모는 자식이 끝날 때까지 기다리지 않고 루프를 실행하기 때문에 프롬프트가 바로 나오는 것이 맞다.
